...
class psm_wrapper:
    ...
    def __call__(self, img1, img2):
        ...
        return self.lpips(
            torch.FloatTensor(img1).to(self.device),
            torch.FloatTensor(img2).to(self.device),
        ).item()
...
class fid_wrapper:
    ...
    def __call__(self, pred_imgs, gt_imgs):
        self.fid.reset()
        self.fid.update(
            torch.tensor(rearrange(gt_imgs, "n w h c -> n c w h")), real=True
        )
        self.fid.update(
            torch.tensor(rearrange(pred_imgs, "n w h c -> n c w h")), real=False
        )
        return self.fid.compute().item()


@torch.no_grad()
def get_inception_score(pred_imgs):
    inception = InceptionScore().cuda()
    inception.update(torch.tensor(rearrange(pred_imgs, "n w h c -> n c w h")).cuda())
    return inception.compute()[0].cpu()
...
def get_similarity_metric(img1, img2, method="pair-wise", metric_name="mse", **kwargs):
    ...
    if method == "pair-wise":
        ...
        return get_inception_score(img1)
    else:
